...
class League(models.Model):
    ...
    is_approved = models.BooleanField(default=False)  # Track if league is approved by admin
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='created_leagues')
...
class GameSelection(models.Model):
    ...
    league = models.ForeignKey(League, on_delete=models.CASCADE, null=True, blank=True)  # Associate picks with league
...
class LeagueCreationRequest(models.Model):
    ...
    is_approved = models.BooleanField(null=True, blank=True)  # None = pending, True/False = approved/denied
    reviewed_at = models.DateTimeField(null=True, blank=True)
    reviewed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviewed_league_requests')

    def __str__(self):
        return f"Request by {self.user.username} to create league '{self.name}'"

class LeagueJoinRequest(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='league_join_requests')
